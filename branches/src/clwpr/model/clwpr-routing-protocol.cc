...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::clwpr::RoutingProtocol")
    ...
    .AddAttribute ("AngFact","Angle Weighting Factor",
                   ...
                   MakeDoubleChecker<double>())
    .AddAttribute ("UtilFact","Utilazation Weighting Factor",
                   DoubleValue(1),
...
void RoutingProtocol::IncreaseMacInfo(){
  m_macInfo++;
}
                                        ...
                                        PosAssociationTuple const &ds_tuple)
{
  ...
  angle = AngleComputation(nb_tuple.neighborPosition,
                                  nb_tuple.neighborHeading,
                                  ds_tuple.nodePosition,
                                  ds_tuple.nodeHeading);
  }


  if (m_normFlag){
  weight = m_Dfact * (nb_dist-my_dist)/my_dist +
           m_Afact * angle +
...
double RoutingProtocol::WeightingFunction(PosAssociationTuple const &ds_tuple)
{
  ...
  if (m_normFlag){
  weight = m_Dfact * (my_dist-my_dist)/my_dist +
           m_Afact * angle +
...
///
/// \param  s_node the source node position.
/// \param  d_node the destination node position.
