...
NS_LOG_COMPONENT_DEFINE ("ClwprHna");
...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, clwprNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get(0), clwprNodes.Get(1)));
  ...
  mobility.Install (clwprNodes);

  ClwprHelper clwpr;

  // Specify Node B's csma device as a non-clwpr device.
  clwpr.ExcludeInterface (clwprNodes.Get (1), 2);
  ...
  InternetStackHelper internet_clwpr;
  internet_clwpr.SetRoutingHelper (list);
  internet_clwpr.Install (clwprNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (clwprNodes.Get (0), tid);
  ...
  Ptr<Ipv4> stack = clwprNodes.Get (1) -> GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<clwpr::RoutingProtocol> clwprrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<clwpr::RoutingProtocol> (temp))
        {
          clwprrp_Gw = DynamicCast<clwpr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      clwprrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }
 
  if (assocMethod2)
    {
      // Specify the required associations directly.
      clwprrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("clwpr-hna", devices);
  csma.EnablePcap ("clwpr-hna", csmaDevices, false);
