  ...
  for (int i = 0; i < 8; ++i)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ ( m_success, true, "Frame power test failed for " << modes[i] << " :  : frame was not successfully decoded");
    }
  return ;
}
...
PhySimWifiFramePowerTest::RunSingle (std::string wifiMode)
{
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (powerLT, powerST, tol, "Power of ST != LT (" << powerST << " != " << powerLT << ") in test");
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (powerSIGNAL, powerST, tol, "Power of SIGNAL != ST (" << powerSIGNAL << " != " << powerLT << ") in test");
  ...
  for (int k = 0; k + 400 + 79 <= m_txSamples.size () - 1; k += 80)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL (powerOFDM, powerST, tol, "Power of  OFDM Symbol != ST (" << powerOFDM << " != " << powerST << ") in test");
