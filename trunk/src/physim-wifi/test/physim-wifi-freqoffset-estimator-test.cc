  ...
  for (int i = 0; i < 8; ++i)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ ( m_success, true, "Frequency offset estimator test failed for " << modes[i] << " :  : frame was not successfully decoded");
    }
  return ;
}
...
PhySimWifiFreqOffsetEstimatorTest::RunSingle (std::string wifiMode)
{
  ...
  // Also enable the transformation from IT++ to IEEE notation (disabling also works)
  ...
  // compare the above to ns3::PhySimSimpleChannelEstimator if you comment the line below -- then tests will fail
  ...
  // Make the frequency offset fixed and high
  Config::SetDefault ("ns3::PhySimWifiPhy::FixedPhaseOffset",UintegerValue (15));
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/RxError", MakeCallback (&PhySimWifiFreqOffsetEstimatorTest::PhyRxErrorTrace, this) );
...
void
PhySimWifiFreqOffsetEstimatorTest::PhyRxErrorTrace (std::string context, Ptr<const Packet> p, Ptr<const PhySimWifiPhyTag> tag)
{
  m_success = false;
}
