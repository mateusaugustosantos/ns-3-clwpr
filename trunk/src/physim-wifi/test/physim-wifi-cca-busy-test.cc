...
void
PhySimWifiCcaSimulationExperiment::DoRun (enum TestId number , std::string desc)
{
  ...
  switch (number)
    ...
    case PhySimWifiCcaSimulationExperiment::TEST_1B:
      ...
      m_checker.RegisterProhibitedStatePeriod (m_nodes[2], PhySimWifiPhy::CCA_BUSY, NanoSeconds (1000000000), NanoSeconds (10001000000));
  ...
  // K.K. Edit -- compatible to ns-3.11
	  if (m_failed)
	  {
//		  NS_TEST_ASSERT_MSG_EQ ( false, true, desc);
		  NS_ASSERT_MSG (m_failed, desc);
		  NS_LOG_INFO ("FAIL: " << desc);
		  m_failed = true;
	  }
	  else
	  {
		 NS_LOG_INFO ("PASS: " << desc);
	 }
...
//      if (e.DoRun (t.id))
//        {
//          NS_TEST_ASSERT_MSG_EQ ( false, true, t.desc);
