...
/*
 * Copyright (c) 2005,2006 INRIA, 2009-2010 Stylianos Papanastasiou, Jens Mittag
 *
 ...
 * - PhySimSignalDetector as well as its optimized version PhySimSignalDetectorOpt
 *
...
private:
  ...
  void CancelAllRunningEndPreambleEvents ();
  void CancelAllRunningEndHeaderEvents ();
...
private:
  ...
  std::map<Ptr<PhySimInterferenceHelper::Event>, EventId> m_endHeaderEvents;
  ...
  double m_sampleTime;
  double m_frequency;
  uint32_t m_fixedPhaseOffset;

  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_rxOkTrace;
  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_rxErrorTrace;
  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_headerOkTrace;
  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_headerErrorTrace;
  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_preambleOkTrace;
  TracedCallback<Ptr<const Packet>, Ptr<const PhySimWifiPhyTag> > m_preambleErrorTrace;
