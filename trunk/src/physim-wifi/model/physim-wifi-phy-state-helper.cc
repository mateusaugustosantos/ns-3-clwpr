...
{
  NS_ASSERT (IsStateIdle () || IsStateCcaBusy () || !IsStateSync ());

  // According to the IEEE 802.11 standard Section 17.3.12 we can only do
  // CCA indication to MAC prior to the signal header. So we don't call
  // NotifyRxStart(duration) here
  // NotifyRxStart (syncDuration);
  ...
  switch (GetState ())
    ...
    case PhySimWifiPhy::RX:
    case PhySimWifiPhy::TX:
      NS_FATAL_ERROR ("PhySimWifiPhyStateHelper::SwitchToSync() - Not allowed: we are in RX or TX state.");
      break;
    }
  m_syncing = true;
...
{
  // Since we don't call NotifyRxStart() in SwitchToSync(), we don't have to call the counterpart here
  // NotifyRxEndOk ();
  DoSwitchFromSync ();
}
void
...
PhySimWifiPhyStateHelper::DoSwitchFromSync (void)
{
  ...
  // If we had the situation that 2 frames arrived shortly after each other, such that we
  // have the following constellation:
  //          t=100000ns Packet 1: PREAMBLE_OK
  //          t=102000ns Packet 2: PREAMBLE_OK
  //          t=104000ns Packet 1: HEADER_OK          (SwitchFromPreambleEndOk)
  //          t=106000ns Packet 2: HEADER_ERROR       (SwitchFromPreambleEndError)
  //
  // we would be in RX state at t=106000ns instead of being in SYNC state
  else if (IsStateRx ())
    {
      NS_ASSERT (m_rxing);
      m_syncing = false;
