...
{
  NS_LOG_FUNCTION_NOARGS ();
  if (m_maxPacketDuration.GetNanoSeconds () == 0)
    {
      NS_LOG_DEBUG ("PhySimInterferenceHelper:GetMaximumPacketDuration() computing the maximum packet duration.");
      m_maxPacketDuration = CalculateTxDuration (4095, WifiPhy::GetOfdmRate6Mbps (), WIFI_PREAMBLE_LONG);
    }
  NS_LOG_DEBUG ("PhySimInterferenceHelper:GetMaximumPacketDuration() returning a value of " << m_maxPacketDuration);
  ...
  while (it != m_noiseChunks.end ())
    {
      ...
      // and then we check for the 2nd case
      else if (indexStart >= nextIndex && indexEnd <= lastIndex)
        {
          ...
          uint32_t numSamples = (indexEnd - indexStart);
#endif
          ...
          NS_LOG_DEBUG ("PhySimInterferenceHelper:GetBackgroundNoise() - --> (2) copied " << numSamples << " sample from local index 0 to " << (numSamples - 1));
