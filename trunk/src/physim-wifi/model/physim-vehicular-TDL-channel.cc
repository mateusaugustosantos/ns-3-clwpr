                                        ...
                                        VEHICULAR_DOPPLER_SPECTRUM shape, double freqShift)
{
  ...
  switch (shape)
    ...
    case Classic3dB:
      for (int i = 0; i < F.size (); i++)
        {
          NS_ASSERT (F (i) <= n_dopp); // Otherwise this is undefined
          if (std::fabs (F (i)) < n_dopp)
            {
              double calc = 1 - std::pow ((F (i) + freqShift) / n_dopp, 2);
              if (calc > 0)
  ...
  for (int i = 0; i < noisesamp; ++i)
    {
      x (i) = S (i) * itpp::randn_c ();
      x (Nfft - 1 - i) = S (Nfft - 1 - i) * itpp::randn_c ();
    }
...
Vehicular_TDL_Channel::Vehicular_TDL_Channel (
  const Vehicular_Channel_Specification &channel_spec,
  double sampling_time)
  : init_flag (false),
    Ts (sampling_time)
{
  if (sampling_time != 0.1e-6)
    {
      NS_FATAL_ERROR ("Sampling time must be 100 ns!!");
    }
...
void Vehicular_TDL_Channel::set_channel_profile (
  const Vehicular_Channel_Specification &channel_spec)
{
