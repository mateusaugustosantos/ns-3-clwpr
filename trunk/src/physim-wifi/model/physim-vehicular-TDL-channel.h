...
public:
  ...
  void
  generate (int no_samples, itpp::cvec &output);
  itpp::cvec
  generate (int no_samples);
protected:
  // ! Generator for VEHICULAR_DOPPLER_SPECTRUM spectrum
  void
  generate_Spectrum (int no_samples, itpp::cvec &output,
                     VEHICULAR_DOPPLER_SPECTRUM shape, double freqShift);
  void
  add_LOS (int idx, std::complex<double>& sample);
private:
  IFFTFadingGenerator ();
 ...
 *  Selective Empirical Channel Models for Vehicular Wireless LANsm, 2009.
 */
class Vehicular_TDL_Channel
{
public:
  // ! Constructor with defined Channel_Specification
  Vehicular_TDL_Channel (const Vehicular_Channel_Specification &channel_spec,
  ...
  void
  set_relative_speed (double speed);
  // ! Set channel profile using Channel_Specification
  void
  set_channel_profile (const Vehicular_Channel_Specification &channel_spec);
  ...
  void
  get_channel_profile (itpp::vec &avg_power_dB, itpp::vec &delay_prof) const;
  // ! Return power profile in dB
  itpp::vec
  get_avg_power_dB () const;
  // ! Return delay profile in samples
  itpp::vec
  get_delay_prof () const
  {
    return d_prof;
  }
  ...
  void
  generate (int no_samples, itpp::Array<itpp::cvec> &channel_coeff);
  void
  filter_known_channel (const itpp::cvec &input, itpp::cvec &output,
  ...
  void
  filter (const itpp::cvec &input, itpp::cvec &output,
...
protected:
  Vehicular_Channel_Specification channel;
