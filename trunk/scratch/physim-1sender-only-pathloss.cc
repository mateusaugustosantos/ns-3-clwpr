...
public:
  ...
  Run (std::string wifiMode, uint32_t seed, double txpower, uint32_t packetSize, double simulationTime, double pathLoss, double bgNoise)
  {
    ...
    // Configure PHY layer to reflect IEEE 802.11a pure-OFDM mode
...
private:
  ...
  void
  PhyRxErrorTrace (std::string context, Ptr<const Packet> p, Ptr<const PhySimWifiPhyTag> tag)
  {
    m_rxErrorCount[m_sinr]++;
  }

  void
  PhyHeaderErrorTrace (std::string context, Ptr<const Packet> p, Ptr<const PhySimWifiPhyTag> tag)
  {
  ...
  void
  PhyPreambleErrorTrace (std::string context, Ptr<const Packet> p, Ptr<const PhySimWifiPhyTag> tag)
  {
